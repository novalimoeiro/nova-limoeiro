...
export default function Assistencia() {
  ...
  const [visitors, setVisitors] = useState({}) // date -> count
  const [loading, setLoading] = useState(true)
  const [attModal, setAttModal] = useState(null)
  const [evtModal, setEvtModal] = useState(null)
  const [visitorModal, setVisitorModal] = useState(null) // {date, day}
      ...
      <div style={{ flex: 1, overflow: 'auto' }}>
        {isLoading ? (
          ...
          <table style={{ borderCollapse: 'collapse', fontSize: 11, width: 'max-content', minWidth: '100%' }}>
            <thead>
              <tr>
                <th style={thSticky(0, 120)}>Membro</th>
                <th style={thSticky(120, 90)}>Grupo</th>
                {meetings.map(mt => {
            ...
            <tbody>
              ...
              <tr>
                <td style={{ ...tdSticky(0, 120), fontWeight: 500, color: 'var(--amber-tx)' }}>Visitantes</td>
                <td style={tdSticky(120, 90)} />
                {meetings.map(mt => {
                  const d = dateStr(mt.date)
                  ...
                  return (
                    <td key={d} style={{ ...tdBase, cursor: 'pointer', background: count ? '#FFF8ED' : undefined }}
                      onClick={() => { setVisitorModal({ date: d, day: mt.date.getDate() }); setVisitorInput(count ? String(count) : '') }}>
                      {count ? <span style={{ fontSize: 11, fontWeight: 500, color: 'var(--amber-tx)' }}>{count}</span> : ''}
                    </td>
                  )
                })}
              ...
              <tr style={{ background: 'var(--bg2)' }}>
                <td style={{ ...tdSticky(0, 120, true), fontSize: 10 }}>Total</td>
                <td style={tdSticky(120, 90, true)} />
                {meetings.map(mt => {
                  const d = dateStr(mt.date)
      ...
      {visitorModal && (
        <div className="modal-overlay" onClick={() => setVisitorModal(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="sub">Informe a quantidade de visitantes nesta reunião.</div>
            <div className="field">
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button className="btn btn-primary" style={{ flex: 1, justifyContent: 'center' }}
                onClick={() => saveVisitors(visitorModal.date, visitorInput)}>
                Salvar
              </button>
              <button className="btn" style={{ flex: 1, justifyContent: 'center' }} onClick={() => setVisitorModal(null)}>
                Cancelar
              </button>
            </div>
          </div>
