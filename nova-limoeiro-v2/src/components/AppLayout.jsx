...
export default function AppLayout() {
  ...
  useEffect(() => {
    if (!localStorage.getItem('theme')) document.documentElement.setAttribute('data-theme', 'light')
  }, [])
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        <header style={{ height: 'var(--header-h)', display: 'flex', alignItems: 'center', padding: '0 16px', borderBottom: 'var(--border)', background: 'var(--bg)', gap: 12, flexShrink: 0 }} className="mobile-header">
          <button onClick={() => setSideOpen(true)} style={{ background: 'none', border: 'none', color: 'var(--text)', fontSize: 22 }}><i className="ti ti-menu-2" /></button>
